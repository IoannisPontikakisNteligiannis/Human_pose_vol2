...
class ExerciseState(Enum):
    ...
    NEUTRAL = "neutral"
    UP_PHASE = "up_phase"  # Concentric (lifting)
    DOWN_PHASE = "down_phase"  # Eccentric (lowering)
...
class BicepCurlDetector:
    ...
    def __init__(self, arm='right'):
        """
        Initialize bicep curl detector 
        ...
        self.arm = arm.lower()
        self.angle_key = f"{self.arm}_elbow"
        
        # Thresholds
        self.min_angle_threshold = 50   # Starting position 
        self.max_angle_threshold = 180  # Peak position
        ...
        self.sustained_frames_slow = 3  # For slow movements 
        self.up_movement_count = 0      # Count consecutive up movements
        self.down_movement_count = 0    # Count consecutive down movements
        self.slow_movement_threshold = 5 # Per-frame movement for slow detection
        ...
        self.min_phase_duration = 0.1   # Minimum time in each phase
        
        # Current movement tracking 
        self.current_movement_direction = 0  # -1, 0, or 1
        self.movement_confirmation_threshold = 1.5 
        self.cumulative_movement = 0  # Track total movement over time for slow detection
    ...
    def _get_sustained_movement_direction(self) -> int:
        ...
        Works for both fast and slow movements
        ...
        # Fast movement: larger per-frame changes
        fast_up_moves = sum(1 for change in recent_changes if change > 3)
        fast_down_moves = sum(1 for change in recent_changes if change < -3)
        
        
        slow_up_moves = sum(1 for change in recent_changes if change > 0.1)  
        slow_down_moves = sum(1 for change in recent_changes if change < -0.1)  
        ...
        if len(recent_changes) > 0:
            ...
            if abs(avg_change) > 0.1:  
                ...
                self.cumulative_movement *= 0.95  # Gradual decay
        ...
        elif slow_up_moves >= 2 and self.cumulative_movement > 1.5:  
            self.up_movement_count += 1
            self.down_movement_count = 0
            return 1
        elif slow_down_moves >= 2 and self.cumulative_movement < -1.5:  
            self.down_movement_count += 1
            self.up_movement_count = 0
            return -1
        else:
    ...
    def _detect_state_change(self, current_angle: float) -> ExerciseState:
        ...
        # Enforce minimum time in each phase to prevent rapid switching
        ...
        if abs(self.cumulative_movement) > self.movement_confirmation_threshold * 2:
            min_time_required *= 0.5  # Reduce time requirement for very clear movements
        
        time_in_current_state = current_time - self.state_change_time
        if time_in_current_state < min_time_required:
            return self.state
        
        # State transition logic based on sustained movement
        if self.state == ExerciseState.NEUTRAL:
            # Only start UP phase with confirmed upward movement
            if (movement_direction == 1 and 
                self._is_movement_confirmed(1) and 
                current_angle > self.min_angle_threshold + 10):
                return ExerciseState.UP_PHASE
                
        elif self.state == ExerciseState.UP_PHASE:
            # Only go to DOWN phase with confirmed downward movement OR at peak
            ...
            # Go to NEUTRAL when back at starting position
            if current_angle <= self.min_angle_threshold + 5:
    ...
    def update(self, angles_dict: Dict[str, Optional[float]]) -> Dict:
        ...
        if raw_angle is None:
            return {
                ...
                'state': 'No Detection',
                'angle': None,
                'feedback': f'{self.arm.title()} arm not visible',
                'debug_info': 'Camera cannot see arm'
            }
    ...
    def _handle_state_transition(self, new_state: ExerciseState, angle: float, current_time: float):
        ...
        if new_state == ExerciseState.UP_PHASE and self.state == ExerciseState.NEUTRAL:
            ...
            print(f"DEBUG: Started new rep at angle {angle:.1f} ({self.arm} arm)")
            
        elif new_state == ExerciseState.UP_PHASE and self.state == ExerciseState.DOWN_PHASE:
            # Going back up 
            print(f"DEBUG: Continuing upward movement at {angle:.1f} ({self.arm} arm)")
            ...
            print(f"DEBUG: Peak reached at {self.peak_angle:.1f}, starting descent ({self.arm} arm)")
                
        elif new_state == ExerciseState.NEUTRAL and self.state == ExerciseState.DOWN_PHASE:
            # Completed rep
            if self.current_rep:
                # Check for minimum range of motion
                range_of_motion = (self.peak_angle or 0) - (self.valley_angle or angle)
                if range_of_motion > 50:  # Minimum valid range
                    ...
                    self.rep_count += 1
                    print(f"DEBUG: Rep {self.rep_count} completed! Range: {self.valley_angle:.1f} to {self.peak_angle:.1f} ({self.arm} arm)")
                else:
                    print(f"DEBUG: Movement too small to count as rep (range: {range_of_motion:.1f}) ({self.arm} arm)")
                self.current_rep = None
        ...
        self.up_movement_count = 0
        self.down_movement_count = 0
        self.cumulative_movement = 0  # Reset cumulative tracking
    ...
    def reset(self):
        ...
        print(f"Reset {self.arm} arm exercise detector")