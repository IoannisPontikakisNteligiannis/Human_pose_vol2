...
class FPSCounter:
    ...
    def __init__(self, averaging_window=10):
        ...
        # Track background processing times
    ...
    def log_data_point(self, frame_type='background_processing', custom_time_ms=None):
        """
        Log a custom data point (e.g., background processing time)
        
        Args:
            frame_type: Type of measurement
            custom_time_ms: Pre-calculated time in milliseconds
        """
        curr_time = time.time()
        
        if frame_type == 'background_processing' and custom_time_ms is not None:
            # Store background processing time
            self.bg_processing_times.append(custom_time_ms)
            if len(self.bg_processing_times) > self.averaging_window:
                self.bg_processing_times.pop(0)
            
            # Log it to history
            self.fps_history.append({
                'frame': self.frame_count,
                'timestamp': curr_time - self.session_start_time,
                'fps': 1000.0 / custom_time_ms if custom_time_ms > 0 else 0,
                'dt': custom_time_ms / 1000.0,
                'frame_time_ms': custom_time_ms,
    ...
    def save_fps_data(self, filename=None):
        ...
        # Store FPS data in a clean format
        fps_data = []
    ...
    def print_detailed_stats(self):
        ...
        if self.bg_processing_times:
            avg_bg = sum(self.bg_processing_times) / len(self.bg_processing_times)
            min_bg = min(self.bg_processing_times)
            max_bg = max(self.bg_processing_times)
            print("-" * 30)
            print(f"Background Processing Frames: {len(self.bg_processing_times)}")
            print(f"Avg Background Time: {avg_bg:.1f}ms ({1000/avg_bg:.1f} FPS)")
            print(f"Min/Max Background Time: {min_bg:.1f}ms / {max_bg:.1f}ms")
